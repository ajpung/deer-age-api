...
function mycred_pro_no_negative_balance( $reply, $request ) {
    ...
    $type = isset($request['type']) ? $request['type'] : 'mycred_default';
    
    if ( !$user_id ) return $reply;
    
    // Check if user has enough credits
    $mycred = mycred($type);
    if ( !$mycred ) return $reply;
...
function get_user_credit_balance() {
    if (!is_user_logged_in() || !function_exists('mycred')) return 0;
    $user_id = get_current_user_id();
    $mycred = mycred('mycred_default');
    return $mycred->get_users_balance($user_id);
}

// ---------------------------------------------------------------------
//                 DEER DATA STORAGE / CREDIT HANDLING                 -
// ---------------------------------------------------------------------
// Create submissions table
function create_deer_submissions_table() {
    global $wpdb;
    
    $table_name = $wpdb->prefix . 'deer_submissions';
...
add_action( 'after_switch_theme', 'create_deer_submissions_table' );

// Deduct credits via AJAX  
add_action('wp_ajax_deduct_deer_credits', 'deduct_deer_credits');
function deduct_deer_credits() {
    if (!is_user_logged_in()) wp_die();
    
    $user_id = get_current_user_id();
    $mycred = mycred('mycred_default');
    $amount = intval($_POST['amount']);
    $predicted_age = isset($_POST['predicted_age']) ? floatval($_POST['predicted_age']) : 1.5; // Default age if missing
    $model_type = isset($_POST['model_type']) ? sanitize_text_field($_POST['model_type']) : 'trailcam'; // Default model if missing
    ...
    // Deduct credits
    $mycred->add_creds('deer_analysis', $user_id, -$amount, 'Deer analysis');
    $new_balance = $mycred->get_users_balance($user_id);
    
    // Always try to store data (removed the conditional check)
    global $wpdb;
    $table_name = $wpdb->prefix . 'deer_submissions';
    
    // Force table creation
    create_deer_submissions_table();
...
// Make credits available to JavaScript
add_action('wp_enqueue_scripts', 'enqueue_credit_data');
function enqueue_credit_data() {
    if (is_page()) {
        wp_localize_script('jquery', 'deerCredits', array(
...
// Use AJAX to get user credits
add_action('wp_ajax_get_user_credits', 'ajax_get_user_credits');
function ajax_get_user_credits() {
    wp_send_json(['credits' => get_user_credit_balance()]);
}


// Redirect new users to home page after registration
function redirect_after_registration($redirect_to) {
    return home_url();
}
add_filter('woocommerce_registration_redirect', 'redirect_after_registration');

// Add to your theme's functions.php
function refund_deer_credits() {
    ...
    if ($amount > 0) {
        mycred_add('analysis_refund', $user_id, $amount, 'Analysis failed - credit refund');
...
add_action('wp_ajax_refund_deer_credits', 'refund_deer_credits');
    ...
    if (is_user_logged_in()) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (menu) {
                ...
                creditsLi.innerHTML = '<a href=\"#\" style=\"pointer-events: none; color: {$color}; font-weight: bold;\">{$balance} Credits</a>';
                menu.insertBefore(creditsLi, menu.firstElementChild);
            }
        });
        </script>";
    }
}
add_action('wp_footer', 'add_credits_script');


// ---------------------------------------------------------------------
//                        NEWSLETTER CHECKBOX                          -
// ---------------------------------------------------------------------
add_action('woocommerce_checkout_before_terms_and_conditions', 'add_newsletter_checkbox');
function add_newsletter_checkbox() {
    echo '<p class="form-row form-row-wide">
        <label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox">
        <input type="checkbox" class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox" name="newsletter_signup" id="newsletter_signup" value="1">
        <span class="woocommerce-form__label-text">Subscribe to our newsletter</span>
        </label>
    </p>';
}

add_action('woocommerce_checkout_update_order_meta', 'save_newsletter_signup');
function save_newsletter_signup($order_id) {
    if (isset($_POST['newsletter_signup'])) {
        $order = wc_get_order($order_id);
        $email = $order->get_billing_email();
        
        global $wpdb;
        $wpdb->insert($wpdb->prefix . 'newsletter', array('email' => $email, 'status' => 'C'));
    }
}
...
function redirect_my_account_to_dashboard() {
    if ( is_account_page() && !is_user_logged_in() ) {
        return; // Let login page work normally
    }
...
//                        DASHBOARD DEER DATA                          -
// ---------------------------------------------------------------------
// Get user deer statistics
function get_user_deer_stats($user_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'deer_submissions';
    ...
    $this_month = $wpdb->get_var($wpdb->prepare(
        "SELECT COUNT(*) FROM $table_name 
         WHERE user_id = %d AND YEAR(submission_date) = YEAR(NOW()) 
         AND MONTH(submission_date) = MONTH(NOW())", $user_id
    ));
    
    // Age distribution for pie chart
    $ages = $wpdb->get_results($wpdb->prepare(
        "SELECT predicted_age, COUNT(*) as count 
         FROM $table_name WHERE user_id = %d 
         GROUP BY predicted_age ORDER BY predicted_age", $user_id
    ));
...
add_action('wp_ajax_get_dashboard_stats', 'ajax_get_dashboard_stats');
function ajax_get_dashboard_stats() {
    if (!is_user_logged_in()) {
        wp_send_json_error('Not logged in');
    }
    
    $stats = get_user_deer_stats(get_current_user_id());
    wp_send_json_success($stats);
}

// Debug: Log when deer data is stored
function debug_deer_submission($user_id, $predicted_age, $model_type, $credits_used) {
    error_log("DEER DEBUG: Storing - User: $user_id, Age: $predicted_age, Model: $model_type, Credits: $credits_used");
}

// Add this temporarily to your functions.php for debugging
add_action('wp_footer', 'debug_table_check');
function debug_table_check() {
    if (is_user_logged_in() && current_user_can('administrator')) {
        global $wpdb;
        $table_name = $wpdb->prefix . 'deer_submissions';
        $exists = $wpdb->get_var("SHOW TABLES LIKE '$table_name'");
        echo "<script>console.log('Table exists: " . ($exists ? 'Yes' : 'No') . "');</script>";
    }
}